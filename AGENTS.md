...
Playback highlighting must honor the engraved duration of those collapsed notes. A note that consumes several silent source slots remains the current highlighted note throughout all of those slots; otherwise a correctly engraved quarter note appears to go blank after its first sixteenth. Store the consumed-slot span on the `StaveNote` and map its rendered SVG elements into every corresponding `drumStepElements` entry. Explicit rests remain unhighlighted.
...
`projects/piano-flashcards/` is a separate endless note-identification trainer. Its three mutually exclusive modes ask with a letter in the middle-C octave, a single note on a complete grand staff, or an audible pitch with no visual answer clue. The staff and ear pools currently span MIDI 36–84, including chromatic notes; the letter pool is restricted to the seven natural notes from middle C through B so an unqualified letter remains unambiguous. The on-screen keyboard covers the same C2–C6 range and labels only octave C keys.

Both an on-screen key and a Web MIDI note-on answer the active card. MIDI setup exposes separate input and output selectors. When an output is selected, ear questions and clicked-key feedback are sent as channel-1 MIDI notes to that device; otherwise a short browser-synthesized piano-like tone is used. Bluetooth pairing remains an operating-system responsibility. Correct answers advance automatically, while wrong answers remain on the same card and highlight the expected key. Session statistics are intentionally ephemeral. Keep random selection and accuracy helpers in `flashcard-core.js`, avoid immediate note repetition, and run `node projects/piano-flashcards/flashcard-core.test.mjs` whenever its question-pool behavior changes.

The flashcard app shares the existing theme storage key and VexFlow version but has its own UI and state machine. Its minimum handheld target is also an iPhone 14 in landscape orientation: preserve safe-area padding, the full keyboard at the bottom, compact controls, and a prompt region that can fit the complete grand staff without page scrolling.
