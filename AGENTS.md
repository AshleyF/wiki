...
`projects/triplet-vocabulary-trainer/` is a standalone scored practice tool with two modes. Vocabulary mode is the default and uses the nine core two-triplet melodies documented in `pages/music/triplet-vocabulary.md`; keep that page as the canonical list: `ABB ABB`, `ABA ABB`, `AAB ABB`, `ABB ABA`, `ABA ABA`, `AAB ABA`, `ABA BAB`, `ABB AAB`, and `AAB AAB`. Triplets mode exposes all eight binary triplet cells from `000` through `111` as selectable miniature engravings, then independently draws twelve cells from the enabled set. Repeats are valid random results. Its three cards are complete 4/4 bars with four triplet cells apiece. Match the wiki's reduced engraving: only `A`/`1` strokes appear on the single line, while `B`/`0` remains an audible ghost stroke. The score cards omit redundant clefs and time signatures, and only the active card receives a state highlight.
