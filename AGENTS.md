...
Long-running MIDI practice tools share `projects/shared/screen-wake-lock.js`. It wraps the standard Screen Wake Lock API, releases the lock when the document is hidden, reacquires it on return while the practice state remains active, and silently degrades when the API is unavailable. The Piano Reading Trainer holds it while running or paused awaiting the correct note; the Triplet Vocabulary Trainer holds it only during playback; Piano Flashcards holds it while either MIDI port is selected. Do not replace it with hidden looping video or duplicate wake-lock lifecycle code in individual projects. Run `node projects/shared/screen-wake-lock.test.mjs` when changing it.

## Shared behavior and change propagation

Prefer one shared implementation whenever two pages or tools are intended to render, play, or behave the same way. Before recreating a renderer, control, scheduler, parser, or interaction in a standalone project, search the repository for an existing implementation that can be extracted into a DOM-independent module or otherwise reused. A shared dependency is preferable to silently divergent copies when the product requirement is parity.

If duplication is genuinely necessary, document the canonical implementation and list every duplicated consumer beside it in this file. Any later bug fix or behavior change must audit and verify every listed consumer in the same change. Do not tell a maintainer that a fix applies “everywhere” unless the behavior is shared or every copy has actually been inspected and tested.
...
VexFlow renders the score and the parsed event list drives playback and note highlighting. Each block's tempo control starts from the source `tempo` directive and remains a playback-only override. Both browser and MIDI playback use a 100 ms rolling lookahead, so live tempo changes affect the next unscheduled interval without restarting the song. Browser audio is the fallback; while the score's shared MIDI connection is enabled and an output is selected, Play sends channel-1 note events to that output instead. Stop must cancel the scheduler, clear the output queue, send explicit note-offs for pitches used by the performance, and send All Sound Off plus All Notes Off so Reset responds promptly even with a virtual MIDI destination. Ordinary note gates use 96% of their written duration: this leaves repeated pitches articulated without turning quarter notes into an audibly staccato sequence. Playback normally keeps the current event centered horizontally and vertically, but wheel, touch, pointer, tempo, or keyboard scrolling interaction suspends automatic scrolling for a short quiet period; continued interaction extends that period. Follow mode is deliberately untimed: it skips written rests, highlights the next playable event, and advances only when the selected MIDI input supplies the exact pitch or chord. Reset stops automatic playback and returns the cursor to the first playable event; when invoked during Follow, it restarts Follow from that event. The selected input shares the Piano Reading Trainer's `piano-reading-trainer-midi-input` preference, and the selected output persists under `personal-wiki-piano-score-midi-output`. Starting another wiki audio block, navigating, hiding the document, or blurring the window must stop score playback and following cleanly without disabling the MIDI connection.
...
Notation layout is selected once for the complete `drums` fence. If exactly one instrument row has visible hits, render a one-line staff; audible-but-hidden `_`, `~`, and `~>` events on any row do not disqualify it. If every visible hit also has a single `R` or `L` sticking, place right-hand noteheads above the line and left-hand noteheads below it, place flam/drag grace notes on the opposite side, and omit the now-redundant sticking letters. If sticking is absent or incomplete, keep every visible notehead on the line and retain any available labels. Any visible hit from a second instrument anywhere in the fence—including one crash in an otherwise snare-only phrase—forces the entire fence to use the full five-line percussion staff; never switch layouts bar by bar. The shared eligibility and one-line geometry helpers live in `projects/rhythm-explorer/drum-notation-core.js`, and the wiki renderer plus `reduced-triplet-renderer.js` must use them rather than recreating the rules.
...
Full-staff drum notation renders begin/end repeat barlines by default, matching playback behavior: every drum sample loops until stopped. Simplified one-line rudimental engraving deliberately omits those repeat barlines: without a full staff, the paired repeat dots look like stray or incomplete symbols. Its playback still loops until stopped. Do not remove repeat signs from full-staff examples unless playback behavior is also changed or the source gains an explicit repeat directive.
...
`projects/piano-flashcards/` is a separate endless note-identification trainer. Letter, staff, and ear are independent prompt channels rather than mutually exclusive modes; any combination may be active, but the UI must prevent disabling the final channel. The channel selection persists separately under `piano-flashcard-prompt-modes`. When ear is combined with a visual channel, play the question tone and show the letter and/or notation instead of also displaying the otherwise redundant ear graphic. One shared note pool controls all prompt combinations and persists under `piano-flashcard-settings`; switching prompts must never replace it. The Notes disclosure contains an 88-key A0–C8 miniature keyboard, and every white and black key can be selected independently. A pointer drag paints or erases only the key color on which the gesture began, so swiping across the upper black-key area does not inadvertently alter white keys. Presets remain quick ways to select natural-note staff and octave ranges from C1 through C7, while the full-piano preset selects all 88 chromatic notes. The picker distinguishes the remembered practice range from its active drill subset: inactive range notes remain gray and active notes are orange. `Random 4` chooses four well-spaced notes from that range, `−1` and `+1` remove or add random active notes, and `All` restores the whole range. The active set may never be empty: erasing stops at one note, persisted empty data repairs itself on load, and `Reset` restores the seven middle-C natural notes instead of clearing everything. Controls that cannot change the current state remain disabled. This supports gradually learning a large arbitrary selection without turning the selected span itself into a clue. Bass-only selections engrave a bass staff, treble-only selections engrave a treble staff, and selections spanning middle C engrave the complete grand staff. The default is the seven natural notes from middle C through B. Letter prompts include the octave number when the selection extends beyond that default octave. The answering keyboard also covers the complete 88-key A0–C8 range and labels only octave C keys.

Both an on-screen key and a Web MIDI note-on answer the active card. MIDI setup exposes separate input and output selectors. When an output is selected, ear questions are sent as channel-1 MIDI notes to that device; otherwise a short browser-synthesized piano-like tone is used. On-screen keys use press-and-release semantics for both output paths: note-on begins on pointer/keyboard down, sustains for as long as the key is held, and note-off (or a synthesized release) occurs on pointer/keyboard up. A short tap uses the same approximately 850 ms decaying envelope as an ear-question note; do not hold it at the sustain level for 850 ms and fade afterward. A longer press sustains until release, then fades over at least 160 ms rather than stopping abruptly. MIDI note-off relies on the connected instrument's own release envelope. Window blur, document hiding, rerendering the keyboard, and changing MIDI ports bypass the minimum tail and release every held note immediately so nothing can stick. Bluetooth pairing remains an operating-system responsibility. Correct answers hold their green prompt/key feedback for 650 ms before advancing; keep this pause long enough to read and to separate an ear-training answer from the following question tone. Wrong answers remain on the same card, turn the central prompt red, and highlight the expected key without playing an error sound. On a first visit the horizontally scrollable keyboard starts with middle C centered. Changing the remembered Notes range clears any manual positioning and recenters on that range's midpoint; changing only its active subset does not. Manual horizontal scrolling is persisted under `piano-flashcard-keyboard-scroll` and restored across reloads until a new range is chosen. Subsequent question following may minimally scroll farther when necessary to keep a target visible, but those automatic movements must not overwrite the user's stored manual position. Session statistics are intentionally ephemeral. Keep random selection and accuracy helpers in `flashcard-core.js`, avoid immediate note repetition, and run `node projects/piano-flashcards/flashcard-core.test.mjs` whenever its question-pool behavior changes.

MIDI-capable pages use independent input and output selectors, as applicable, with `None` as the first option and no separate enable checkbox or button. Opening a dedicated MIDI disclosure—or focusing an always-visible selector—requests Web MIDI. Persist explicitly selected port IDs per tool and restore them only when those ports are available; never auto-select the first discovered port. Input-only selection must leave browser audio active, while selecting an actual output suppresses browser audio only on tools where MIDI replaces playback. Choosing `None` detaches that direction without disabling the other direction.
...
## Triplet Vocabulary Trainer

Accuracy and the visible hit/miss totals extend only through the player's most recent actual tap or accepted MIDI input. Expected strokes that expire after that input remain provisional: if the player resumes before recovery, commit all provisional misses immediately before scoring the new attempt; if silence reaches recovery, discard them so merely stopping does not lower the completed run's score.

`projects/triplet-vocabulary-trainer/` is a standalone scored practice tool with six modes, ordered in the UI as Triplets, Basic Vocabulary, Extended Vocabulary, Kick 1, Kick 2, and Kick 1 + 2. Basic Vocabulary remains the default for a new browser profile and uses the nine core two-triplet melodies documented in `pages/music/triplet-vocabulary.md`; keep that page as the canonical list: `ABB ABB`, `ABA ABB`, `AAB ABB`, `ABB ABA`, `ABA ABA`, `AAB ABA`, `ABA BAB`, `ABB AAB`, and `AAB AAB`. Extended Vocabulary has three continuous 4/4 cards, each containing four triplets. Two selectors construct every card: its first two triplets choose among `ABB ABA`, `ABA ABA`, and `AAB ABA`, while its last two choose among `RAB ABB`, `RAB ABA`, `RAB AAB`, `RBA ABB`, `RBA BBA`, and `RBA ABA`, where the leading `R` is a true eighth-note rest and every `B` remains an audible hidden ghost. Compact UI labels print that rest as `–`. Extended pattern numbers are local to that mode and run 1–9 in the order just listed. Its filter must retain at least one enabled choice in both the 1–3 and 4–9 groups. Manual scrambling replaces all three cards; automatic scrambling replaces each completed card with a fresh valid pair. Triplets mode exposes all eight binary triplet cells from `000` through `111` as selectable miniature engravings, then independently draws twelve cells from the enabled set. Repeats are valid random results. Its three cards are complete 4/4 bars with four triplet cells apiece.

The three Kick modes reuse the nine Vocabulary filters but orchestrate every visible `A` as kick and every hidden `B` as a snare ghost under closed hi-hat quarter notes. Kick 1 places one selected two-triplet melody on beats 1–2, an accented snare on beat 3, and hi-hat alone on beat 4. Kick 2 fixes kick on beat 1 and hi-hat alone on beat 2, then treats the accented snare on beat 3 as the first stroke of one selected two-triplet melody and puts that melody's remaining five positions on kick. Kick 1 + 2 composes those variable halves: its first selector supplies the complete Kick 1 melody on beats 1–2, and its second selector supplies the Kick 2 melody beginning with the accented snare on beat 3. Each selection is independent, including during manual and automatic scrambling. All three use complete four-beat cards and the same full percussion-staff renderer. Hi-hat stays in the stems-up groove voice; kick and Kick 2's pattern-defining snare stay in the stems-down vocabulary voice. In every mode, the optional emphasized pattern receives exactly half of its eligible random range; the remaining half is distributed among the other enabled choices. Filter buttons use compact numbers while selectors retain full pattern labels. Vocabulary, Extended, and Triplets match the wiki's reduced engraving: only `A`/`1` strokes appear on the single line, while `B`/`0` remains an audible ghost stroke. Their score cards omit redundant clefs and time signatures; Kick cards show the full staff, with the percussion clef and 4/4 signature visible only on the first card while all three reserve identical notation width. Only the active card receives a state highlight.

The trainer must remain full viewport width on an iPhone, including portrait use with reduced vertical room for browser chrome. Keep every form control at a computed font size of at least 16 pixels at mobile widths so iOS does not auto-zoom after a mode selection. The page shell, main grid, phrase, and cards need explicit full-width/min-width-zero constraints; only the pattern-filter strip should scroll horizontally. The reduced single-line score SVG is deliberately 90 pixels tall: the stems-up engraving occupies its upper portion while the remaining lower room accommodates optional count annotations. Kick mode uses 148 pixels for its two-voice percussion staff and lower triplet brackets, expanding to 168 pixels only when Count needs a separate baseline beneath those brackets. Keep the three-card gap and outer section spacing compact. Render the notation on the animation frame after switching modes so widths are measured after the layout has settled. The ordinary page uses native touch scrolling without any custom locking or momentum. Non-control touches still count as practice input when they end without crossing the 18-CSS-pixel tap-versus-drag threshold, using their original touch-down time. Starting playback from the compact lower transport expands the score panel to roughly two-and-a-half times its resting height, reveals a dedicated `touch-action: none` tap surface beneath the statistics, and scrolls it into view. That dedicated surface is the only area where dragging must never scroll. Stopping collapses it. Do not apply practice gestures to buttons, inputs, selects, links, or labels.

The trainer's three velocity handles persist together under `triplet-vocabulary-velocities`. Restore only a complete, strictly ordered set of three integer MIDI velocities from 1–127; otherwise use the defaults `16`, `64`, and `111`. Persist continuously as a handle moves, while retaining the existing behavior of preparing and latching new sample layers only after the completed change event.

Scored playback has an automatic recovery state, but it remains dormant until the session receives its first eligible screen tap or MIDI hit. After that, four consecutive sounded scoring targets (`A` strokes, plus Kick mode's `S` landing) that expire unanswered start a rolling recovery transition without resetting the audio clock. Silent triplet positions and explicit rests are never recovery misses, so a sparse phrase may take longer to enter recovery than a dense one. The card currently under the cursor remains unchanged; the other two cards temporarily render and play only a quarter-note snare pulse (`A R R` per beat), and the preserved card becomes a recovery card only after playback crosses its boundary. Engraving in every trainer mode must derive from `selectedPattern()`, the same recovery-aware effective pattern used by scheduling; do not let Triplets mode read `tripletCards` directly or its display will remain stale while audio enters recovery. Entering recovery freezes the visible score so the player can still see the interrupted run's result; recovery guide strokes and attempts do not alter it. Re-entry requires two consecutive correctly timed recovery hits for Vocabulary's two-beat cards and four for the four-beat Extended, Triplets, and Kick cards. During recovery, three consecutive quarter-note attempts with tightly clustered raw timing offsets implicitly recalibrate latency: persist their median offset, credit those attempts toward the recovery streak, and continue from the newly aligned grid. Retain recovery targets long enough to observe Bluetooth-scale output latency, but do not learn from irregular or nonconsecutive attempts. Confirmation resets the visible score, leaves the current card as recovery material through its boundary, and immediately prepares normal material in the previous and next cards so the following card resumes the exercise without an additional full recovery bar. Normal scoring begins from zero after the current card crosses its boundary. A missed or extra recovery hit resets only the hidden recovery streak. Defer transition engraving until after the scheduler finishes its lookahead pass so VexFlow cannot stall scheduling and cause overdue hits to fire as a burst. Stopping playback clears recovery immediately and restores the selected material.

Play begins a fresh scored session. The optional Metronome control defaults off; when enabled, it adds a four-quarter count-in plus an invisible pedal-hi-hat quarter-note pulse whose 4/4 downbeat uses the stronger velocity. Kick mode already supplies its own visible closed-hi-hat pulse, so do not layer the ongoing metronome sound over those cards. Visible `A`/`1` strokes and Kick mode's beat-3 snare are scoring targets. Pointer/touch presses anywhere in the main non-control area—including the notation cards and score strip—Space key presses outside ordinary controls, and accepted MIDI note-ons from the selected input must all enter the same `registerPracticeHit()` path. The persistent Ignore feet and Ignore ghosts filters default on. Ignore feet rejects General MIDI acoustic/bass kick notes 35/36 and pedal hi-hat note 44, but it is temporarily disabled and unchecked in Kick mode so the exercise can receive kick input. Kick mode silently filters incoming closed, pedal, and open hi-hat notes 42/44/46 because its visible hat is timekeeping rather than a separate scoring target. Ignore ghosts rejects velocities below the midpoint between the current Ghost and Normal slider handles, without restricting accepted strokes to a particular non-foot instrument. A tiny activity dot beside MIDI In flashes only for note-ons that pass both filters, even when playback is stopped, so routing can be checked without affecting the score. Buttons, inputs, selects, links, and their labels must never count as performance taps. The score strip includes a compact icon-only secondary transport for mobile use; it and the labeled top transport must call the same start/stop path and always display the same state. MIDI output remains independently selectable for Superior Drummer. A tempo-scaled timing window assigns a tap to the nearest still-unclaimed target; it is modestly wider than the original window and allocates two-thirds of its span after the beat to accommodate delayed monitoring such as Apple TV audio. Unmatched taps and targets that pass beyond the late edge count as misses. The score strip updates numbers only: Accuracy, current Streak since the last miss, session Best streak, total Hits, and total Misses. Do not flash, recolor, animate, or print per-hit instructions because those changes distract from sight-reading. Starting playback after a stop resets every statistic. Keep pure filtering, scoring, and triplet-generation rules in `trainer-core.js` and run `node --test projects/triplet-vocabulary-trainer/trainer-core.test.mjs` after changing them.

Do not copy reduced-triplet engraving, single-line geometry, or SVG stem-mapping logic into this project. `projects/rhythm-explorer/drum-notation-core.js` owns the single-instrument eligibility rule, one-line stave/note positions, reduced rhythmic spellings, and detached VexFlow stem discovery. `projects/rhythm-explorer/reduced-triplet-renderer.js` owns the fixed three-column layout, beam construction, and tuplet bracket placement used by both the wiki's eight-combination engraving example and this trainer. Those two consumers must stay on this shared renderer so spacing, single-line layout, and dark-mode fixes propagate together. The trainer also loads the wiki's root `styles.css` and uses its `drum-*` control and notation classes; its local stylesheet should contain only standalone page layout. Run `node --test projects/rhythm-explorer/drum-notation-core.test.mjs projects/rhythm-explorer/reduced-triplet-renderer.test.mjs` when changing the shared rules or renderer.

The compact `Auto` control is a three-slot rolling shuffle and defaults on for a first-time visitor. On each card boundary, the slot just completed becomes the far-future slot and may be shuffled; the active slot and the immediately following slot must never change underneath the player. Each shuffle samples uniformly from the current mode's enabled set, including the value already in that slot—natural random repetitions are allowed. A manual Shuffle request made during playback waits for the end of the complete three-card loop. The always-visible filter row defines the available vocabulary for every selector and shuffle path, persists locally, and must never allow an empty set. Vocabulary cards count `1 & a 2 & a`; Kick cards add `3 & a`; complete Triplets-mode bars count `1 & a 2 & a 3 & a 4 & a`. Hidden B/0 events do not print count labels. The persistent Count checkbox controls those annotations and defaults off. The persistent Follow checkbox controls only the moving note/rest cursor and must use the same shared stem discovery and root `.drum-current-note` styling as wiki drum blocks, including dark-mode behavior; disabling it clears any current cursor immediately. Built-in playback populates its Sound selector from the shared drum-sample manifest and shares the wiki's `personal-wiki-drum-snare-kit` preference; Ludwig Black Beauty is only the fallback default. MIDI output uses percussion channel 10. Existing modes use center snare note 38 and pedal hi-hat note 44; Kick mode adds bass drum note 36 and closed hi-hat note 42.

The persistent Repeat menu defaults to 1 and allows 1–16 performances of every card before the rolling cursor advances, recovery transitions, queued scrambling, or automatic scrambling treats that card as complete. Later passes highlight the same engraved notes again rather than duplicating the notation. Every card shows a compact end-repeat barline whenever the count is greater than 1, including both the shared reduced-triplet renderer and the full-staff Kick renderer. Counts of three or more initially use the conventional static `×N` play-count label above the repeat barline. At each playback jump, the repeat barline flashes and the count changes to the remaining number of playthroughs (`×2`, then `×1`, for an initial `×3`); that countdown stays highlighted while the notes continue highlighting. On advancing to the next card, the completed card's label returns to its unhighlighted original total. A count of 2 keeps the conventional unlabeled repeat barline at rest, but reveals the highlighted `×1` countdown during its second pass. Single-card audition playback follows the same repeat-count rule.

When Repeat is greater than 1, generated cards must not duplicate either adjacent card whenever the enabled pattern pool contains a valid alternative. Apply this to initial/manual scrambling, automatic card replacement, and recovery replacement in every mode. Pattern identity means the complete displayed card: four masks in Triplets, the complete pair in Extended and Kick 1 + 2, and the melody number in the single-pattern modes. Preserve the selected emphasis behavior for the first draw, then fall back to any permitted candidate if that draw duplicates a neighbor. If the enabled pool cannot satisfy both sides of the three-card loop, an adjacent duplicate remains valid rather than inventing or enabling an unselected pattern.
