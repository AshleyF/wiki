...
Play begins a fresh scored session. The optional Metronome control defaults off; when enabled, it adds a four-quarter count-in plus an invisible pedal-hi-hat quarter-note pulse whose 4/4 downbeat uses the stronger velocity. Only the visible `A`/`1` strokes are scoring targets. Pointer/touch presses anywhere in the main non-control area—including the notation cards and score strip—Space key presses outside ordinary controls, and every nonzero MIDI note-on from the selected input must all enter the same `registerPracticeHit()` path. Buttons, inputs, selects, links, and their labels must never count as performance taps. MIDI output remains independently selectable for Superior Drummer. A tempo-scaled timing window assigns a tap to the nearest still-unclaimed target; unmatched taps and targets that pass beyond the late edge count as misses. The score strip updates numbers only: do not flash, recolor, animate, or print per-hit instructions because those changes distract from sight-reading. Its Best streak value retains the longest run within the current session even after a miss; starting playback after a stop resets it and every other statistic. Keep pure scoring and triplet-generation rules in `trainer-core.js` and run `node --test projects/triplet-vocabulary-trainer/trainer-core.test.mjs` after changing them. The main area must retain `touch-action: manipulation` and prevent non-control double-click defaults so repeated mobile taps cannot trigger browser zoom.
