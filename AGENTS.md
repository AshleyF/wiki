...
The persistent Repeat menu defaults to 1 and allows 1–8 performances of every card before the rolling cursor advances, recovery transitions, queued scrambling, or automatic scrambling treats that card as complete. Later passes highlight the same engraved notes again rather than duplicating the notation. Every card shows a compact end-repeat barline whenever the count is greater than 1, including both the shared reduced-triplet renderer and the full-staff Kick renderer. Counts of three or more use the conventional static `×N` play-count label above the repeat barline; the number always means total playthroughs, not remaining playthroughs. During playback the repeat barline and count highlight briefly at each jump before highlighting returns to the first note. Single-card audition playback follows the same repeat-count rule.
