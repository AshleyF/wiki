...
The persistent Repeat checkbox defaults off. When enabled, every card is performed twice before the rolling cursor advances, recovery transitions, queued scrambling, or automatic scrambling treats that card as complete. The second pass highlights the same engraved notes again rather than duplicating the notation. Every card shows a compact end-repeat barline while the option is active, including both the shared reduced-triplet renderer and the full-staff Kick renderer. Single-card audition playback follows the same two-pass rule.
