...
The trainer must remain full viewport width on an iPhone in landscape. Keep every form control at a computed font size of at least 16 pixels at mobile widths so iOS does not auto-zoom after a mode selection. The page shell, main grid, phrase, and cards need explicit full-width/min-width-zero constraints; only the pattern-filter strip should scroll horizontally. Render the notation on the animation frame after switching modes so widths are measured after the Vocabulary/Triplets layout has settled. Because repeated taps are the trainer's primary input, its page-specific viewport disables scaling and the notation and practice-pad tap surfaces use `touch-action: none`. A custom non-passive touch gesture applies an 18-CSS-pixel movement threshold in non-control parts of `main`: movement below it stays latched and becomes one practice hit at the original touch-down time. Crossing the threshold cancels the hit, immediately catches the document scroll position up to the finger's complete displacement, follows subsequent movement one-to-one, and continues with a short decaying inertial glide on release. A new touch cancels an existing glide. Do not apply that gesture to buttons, inputs, selects, links, or labels.
