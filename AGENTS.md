...
`projects/triplet-vocabulary-trainer/` is a standalone scored practice tool with four modes. Vocabulary mode is the default and uses the nine core two-triplet melodies documented in `pages/music/triplet-vocabulary.md`; keep that page as the canonical list: `ABB ABB`, `ABA ABB`, `AAB ABB`, `ABB ABA`, `ABA ABA`, `AAB ABA`, `ABA BAB`, `ABB AAB`, and `AAB AAB`. Extended mode has three continuous 4/4 cards, each containing four triplets. Two selectors construct every card: its first two triplets choose among `ABB ABA`, `ABA ABA`, and `AAB ABA`, while its last two choose among `RAB ABB`, `RAB ABA`, `RAB AAB`, `RBA ABB`, `RBA BBA`, and `RBA ABA`, where the leading `R` is a true eighth-note rest and every `B` remains an audible hidden ghost. Compact UI labels print that rest as `–`. Extended pattern numbers are local to that mode and run 1–9 in the order just listed. Its filter must retain at least one enabled choice in both the 1–3 and 4–9 groups. Manual shuffling replaces all three cards; automatic shuffling replaces each completed card with a fresh valid pair. Triplets mode exposes all eight binary triplet cells from `000` through `111` as selectable miniature engravings, then independently draws twelve cells from the enabled set. Repeats are valid random results. Its three cards are complete 4/4 bars with four triplet cells apiece. Kick mode reuses the nine Vocabulary filters and selectors but extends each two-triplet melody with a third beat: every visible `A` is a kick, every `B` is an audible hidden snare ghost, closed hi-hat marks all three quarter-note pulses, and beat 3 lands with an accented snare. Its three cards use a full 3/4 percussion staff because they contain multiple instruments. The hi-hat and snare form the stems-up groove voice; the kick vocabulary is a separate stems-down voice. Only the first two beats use reduced triplet spellings and tuplet marks; beat 3 is an ordinary quarter-note hi-hat/snare chord, and the silent completion of the kick voice stays hidden. In every mode, the optional emphasized pattern receives exactly half of its eligible random range; the remaining half is distributed among the other enabled choices. Filter buttons use compact numbers while selectors retain full pattern labels. Vocabulary, Extended, and Triplets match the wiki's reduced engraving: only `A`/`1` strokes appear on the single line, while `B`/`0` remains an audible ghost stroke. Their score cards omit redundant clefs and time signatures; Kick cards show the full staff, with the percussion clef and 3/4 signature visible only on the first card while all three reserve identical notation width. Only the active card receives a state highlight.
