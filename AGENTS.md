...
The cursor follows the performance clock rather than note-entry timing. A note or chord is accepted only inside the configured early/late window; an early entry, wrong pitch set, or missed deadline pauses the exercise at the current event and highlights every expected on-screen key as a recovery hint. While paused, both MIDI and the on-screen keyboard remain active: playing the expected pitch set clears those hints, accepts the event, and restarts the clock toward the following event, while another wrong entry leaves the drill paused. Keep the pure timing and pitch-set helpers in `trainer-core.js` and run `node projects/piano/trainer-core.test.mjs` when changing those semantics. Microphone pitch detection is not implemented; if added later, keep its latency calibration and confidence filtering separate from the MIDI path before feeding accepted pitches into the shared attempt classifier.
